...
export default async (client: Manager, player: KazagumoPlayer, track: KazagumoTrack) => {
  ...
  const embeded = new EmbedBuilder()
    ...
    .addFields([
      ...
      { name: `${client.i18n.get(language, "player", "duration_title")}`, value: `${formatduration(song!.length)}`, inline: true },
  ...
  const playing_channel = await client.channels.cache.get(player.textId) as TextChannel

  const nplaying = await playing_channel.send({ embeds: [embeded], components: [row, row2] });
  ...
  const collector = await nplaying.createMessageComponentCollector({ filter, time: song!.length });
