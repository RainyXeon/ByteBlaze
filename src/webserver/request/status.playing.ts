...
export default class implements RequestInterface {
  ...
  run = async (client: Manager, json: JSON_MESSAGE, ws: WebSocket) => {
    ...
    if (player.state == RainlinkPlayerState.DESTROYED)
      return ws.send(JSON.stringify({ op: "player_destroy", guild: player.guildId }));
    else if (player.state == RainlinkPlayerState.CONNECTED)
      return ws.send(JSON.stringify({ op: "player_create", guild: player.guildId }));
  };
}
