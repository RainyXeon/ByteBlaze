...
export class PostCreatePlayer {
  ...
  async main(req: Fastify.FastifyRequest, res: Fastify.FastifyReply) {
    this.client.logger.info(import.meta.url, `${req.method} ${req.routeOptions.url} payload=${req.body ? util.inspect(req.body) : "{}"}`);
  ...
  async checker(data: Record<string, string>, req: Fastify.FastifyRequest, res: Fastify.FastifyReply): Promise<boolean> {
